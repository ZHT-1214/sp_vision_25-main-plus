  ...
  std::vector<std::vector<cv::Point2f>> & img_points)
{
  ...
  auto pattern_cols = yaml["pattern_cols"].as<int>();// 内角点列数
  auto pattern_rows = yaml["pattern_rows"].as<int>();// 内角点行数
  // auto center_distance_mm = yaml["center_distance_mm"].as<double>();
  auto square_size = yaml["square_size"].as<double>();
  ...
  for (int i = 1; true; i++) {
    ...
    // obj_points.emplace_back(centers_3d(pattern_size, center_distance_mm));
      ...
      // 修改后
    img_points.emplace_back(centers_2d);
    obj_points.emplace_back(chessboard_corners_3d(pattern_size, square_size));
  }
}
