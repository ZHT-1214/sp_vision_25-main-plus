...
void Solver::set_camera_extrinsic(
  const Eigen::Matrix3d & rotation, const Eigen::Vector3d & translation)
{
  R_camera2gimbal_ = rotation;
  t_camera2gimbal_ = translation;
}
...
void Solver::solve(Armor & armor) const
{
  ...
  // 平面目标 IPPE 存在两个镜像解，用 solvePnPGeneric 同时求取，再按几何先验消歧
  ...
  // 第 i 个解的 4 角点重投影误差之和
  auto reprojection_error = [&](std::size_t i) {
    std::vector<cv::Point2f> proj;
    cv::projectPoints(object_points, rvecs[i], tvecs[i], camera_matrix_, distort_coeffs_, proj);
    double error = 0.0;
    for (int k = 0; k < 4; ++k) error += cv::norm(armor.points[k] - proj[k]);
    return error;
  };
  ...
  for (std::size_t i = 0; i < rvecs.size(); ++i) {
    if (tvecs[i][2] <= 0) {  // 相机后方剔除
      // tools::logger()->debug(
      //   "[Solver] sol[{}] rejected: z={:.3f} behind camera", i, tvecs[i][2]);
      continue;
    }
    ...
    Eigen::Matrix3d R_armor2camera_i;
    cv::cv2eigen(rmat_i, R_armor2camera_i);

    // 背向（落在相机后方一侧）剔除，只作为几何合理性检查。
    // 本工程把装甲板自身 +x 轴定义为「从装甲板指向旋转中心」（见 reproject_armor 的
    // R_armor2world 第一列），因此法线背向相机的那一支 dot > 0，剔除 dot <= 0 的那支。
    //
    // 注意：实测（demo 视频）IPPE 给出的两个解 dot 几乎相等（如 0.823 vs 0.824），
    // 两者相差的是绕装甲板自身轴的 ~180° yaw/roll，并非关于板面的镜像解。所以这个
    // 判据在本数据上不起区分作用，真正决定选哪个解的是下面的重投影误差。
    Eigen::Vector3d normal = R_armor2camera_i * Eigen::Vector3d::UnitX();
    double dot = normal.dot(xyz_in_camera_i);
    if (dot <= 0) {
      // tools::logger()->debug(
      //   "[Solver] sol[{}] rejected: z={:.3f} dot={:.3f} back-facing", i, tvecs[i][2], dot);
      continue;
    }

    double error = reprojection_error(i);
    // tools::logger()->debug(
    //   "[Solver] sol[{}] ok: z={:.3f} dot={:.3f} err={:.2f}", i, tvecs[i][2], dot, error);
    if (error < best_error) {
      best_error = error;
  ...
  // 所有解都被几何先验剔除（通常意味着检测或标定异常）：
  // 退回到重投影误差最小的解，绝不使用被明确淘汰的解
  if (best < 0) {
    for (std::size_t i = 0; i < rvecs.size(); ++i) {
      double error = reprojection_error(i);
      if (error < best_error) {
    ...
    tools::logger()->warn(
      "[Solver] All PnP solutions rejected (n={}), fallback err={:.2f}", rvecs.size(), best_error);
  }
  if (best < 0) return;  // rvecs 非空时不会走到这里

  const cv::Vec3d & rvec = rvecs[best];
  const cv::Vec3d & tvec = tvecs[best];
  ...
  const Eigen::Vector3d & xyz_in_world, double yaw, ArmorType type, ArmorName name) const
{
  ...
  // auto pitch = (name == ArmorName::outpost) ? -15.0 * CV_PI / 180.0 : 0.0;
...
void Solver::optimize_yaw(Armor & armor) const
{
  ...
  // 注意：这里是无条件覆盖。实验表明「残差过大就不覆盖、保留 PnP 姿态」会**更差**
  // （tracked 541→514，diverged 22→30）：因为 EKF 的观测模型 h_reproject 用的就是
  // reproject_armor 本身，让该模型残差最小的 yaw 才是与滤波器自洽的 yaw —— 哪怕模型
  // 本身有偏。要真正改善只能去修正 ±15° 的 pitch 假设，而不是在这里保留 PnP 的 yaw。
  armor.yaw_raw = armor.ypr_in_world[0];
  armor.ypr_in_world[0] = best_yaw;

  // 诊断：pnp= PnP 解出的 yaw，used= 最终采用的 yaw，residual= 约束模型下的最小残差。
  // residual 长期停在几十~几百像素，说明 ±15° pitch 的假设与这块板不符（或标定/安装不符）。
  auto err_at_raw = armor_reprojection_error(armor, armor.yaw_raw, 0);
  auto pts_used = reproject_armor(
    armor.xyz_in_world, armor.ypr_in_world[0], armor.type, armor.name);
  // tools::logger()->debug(
  //   "[Solver] yaw optimize: pnp={:.1f} used={:.1f} (offset {:+.1f}) residual={:.1f}px "
  //   "pnp_residual={:.1f}px",
  //   armor.yaw_raw * 57.3, armor.ypr_in_world[0] * 57.3,
  //   tools::limit_rad(armor.ypr_in_world[0] - gimbal_ypr[0]) * 57.3, min_error, err_at_raw);
  // tools::logger()->debug(
  //   "[Solver]   det  ({:.0f},{:.0f}) ({:.0f},{:.0f}) ({:.0f},{:.0f}) ({:.0f},{:.0f})",
  ...
  // tools::logger()->debug(
  //   "[Solver]   proj ({:.0f},{:.0f}) ({:.0f},{:.0f}) ({:.0f},{:.0f}) ({:.0f},{:.0f})",
  //   pts_used[0].x, pts_used[0].y, pts_used[1].x, pts_used[1].y, pts_used[2].x, pts_used[2].y,
  //   pts_used[3].x, pts_used[3].y);
}
