  ...
  omni_target_priority_{ArmorPriority::fifth}
{
  ...
  // 马氏距离门控参数（带默认值，其它 config 未配置时不会报错）
  gate_threshold_ = yaml["gate_threshold"].as<double>(13.28);
  gate_yaw_noise_ = yaml["gate_yaw_noise"].as<double>(0.004);
  gate_pitch_noise_ = yaml["gate_pitch_noise"].as<double>(0.004);
  gate_distance_noise_ratio_ = yaml["gate_distance_noise_ratio"].as<double>(0.05);
  gate_angle_noise_ = yaml["gate_angle_noise"].as<double>(0.09);
  armor_height_diff_ = yaml["armor_height_diff"].as<double>(0.0);
  armor_radius_ = yaml["armor_radius"].as<double>(0.2);
  stationary_target_ = yaml["stationary_target"].as<bool>(false);
  lock_stationary_center_ = yaml["lock_stationary_center"].as<bool>(false);
}
...
bool Tracker::set_target(std::list<Armor> & armors, std::chrono::steady_clock::time_point t)
{
  ...
  if (is_balance) {
    ...
    target_ = Target(armor, t, armor_radius_, 4, P0_dig, solver_, armor_height_diff_);
  }

  // 设置马氏距离门控参数（从 config yaml 读入）
  target_.gate_threshold_ = gate_threshold_;
  target_.gate_yaw_noise_ = gate_yaw_noise_;
  target_.gate_pitch_noise_ = gate_pitch_noise_;
  target_.gate_distance_noise_ratio_ = gate_distance_noise_ratio_;
  target_.gate_angle_noise_ = gate_angle_noise_;
  target_.set_stationary(stationary_target_, lock_stationary_center_);
