...
namespace protocal {
    ...
    constexpr uint8_t heartbeat_raw[] { 0xaa, 0x01, 0x0f, 0x00, 0x00, 0x00, 0x00, 0x04,
                                        0xd7, 0x00, 0x03, 0x38, 0xba, 0x8d, 0x0c };
    constexpr uint8_t start_data[] { 0x00, 0x04, 0x01, 0xab, 0xb2, 0x3b, 0x4b };
...
#pragma pack(pop)
    ...
    inline bool check_header(const data_header& header) {
        ...
        if (header.version.value() != 5) {
            RCLCPP_ERROR(
                rclcpp::get_logger("header"),
                "version is not 5, got %u",
        ...
        if (header.data_type.value() != 2) {
            RCLCPP_ERROR(
                rclcpp::get_logger("header"),
                "data_type is not 2, got %u",
                header.data_type.value()
            );
            return false;
        }
        if (header.lidar_id.value() != 1) {
            RCLCPP_ERROR(
                rclcpp::get_logger("header"),
                "lidar_id is not 1, got %u",
                header.lidar_id.value()
            );
            return false;
        }
        return true;
    ...
    inline size_t
    write_frame_buffer(uint8_t* buf, const uint8_t* data, size_t size, size_t max_size) {
        ...
        header.crc_16 = FastCRC::mcrf4xx_calc(
            livox_crc16_init,
            header_data,
            sizeof(header) - sizeof(header.crc_16)
        );
        std::copy(header_data, header_data + sizeof(frame_header), buf);

        std::copy(data, data + size, buf + sizeof(frame_header));

        little_uint32_t crc32_val =
            FastCRC::crc32_calc(livox_crc32_init, buf, sizeof(header) + size);
