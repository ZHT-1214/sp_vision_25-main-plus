...
public:
    ...
    explicit LinearSyncModel(
        ...
        , residual_threshold_sec_ {
              residual_threshold_sec > 0.0 ? residual_threshold_sec : kDefaultResidualThresholdSec } { }
    ...
    [[nodiscard]] static auto fit_least_squares(std::span<const TimestampPair> samples) noexcept
        ...
        auto max_abs_residual_sec = 0.0;
        auto sum_squared_residual = 0.0;
        for (const auto& sample : samples) {
            const auto residual = sample.board_timestamp_sec - (a * sample.camera_timestamp_sec + b);
        ...
        auto covariance = Eigen::Matrix2d { };
    ...
    [[nodiscard]] auto initialize_from_least_squares(std::span<const TimestampPair> samples) noexcept
        -> std::optional<LeastSquaresResult> {
    ...
    [[nodiscard]] auto residual_for(
        const double camera_timestamp_sec, const double board_timestamp_sec) const noexcept -> double {
        return board_timestamp_sec - predict(camera_timestamp_sec);
    }

    auto update(const double camera_timestamp_sec, const double board_timestamp_sec) noexcept -> double {
        const auto regressor = Eigen::Vector2d { camera_timestamp_sec, 1.0 };
        ...
        const auto projected_covariance = covariance_ * regressor;
        ...
        const auto gain = projected_covariance / denominator;
