...
//   "{config-path c  | configs/ascento.yaml | yaml配置文件路径 }"
...
//   auto square_size = yaml["square_size"].as<double>();  // 棋盘格格子边长
...
//     cv::Mat rvec, tvec;
//     auto corners_3d = chessboard_corners_3d(pattern_size, square_size);  // 使用棋盘格3D点
//     cv::solvePnP(
...
std::vector<cv::Point3f> generate_chessboard_corners(const cv::Size & pattern_size, const float square_size)
{
  std::vector<cv::Point3f> corners_3d;
  ...
  std::vector<cv::Mat> & tvecs)
{
  ...
  auto pattern_cols = yaml["pattern_cols"].as<int>();
  auto pattern_rows = yaml["pattern_rows"].as<int>();
  auto square_size_mm = yaml["square_size"].as<double>();
  R_gimbal2imubody_data = yaml["R_gimbal2imubody"].as<std::vector<double>>();
  auto camera_matrix_data = yaml["camera_matrix"].as<std::vector<double>>();
  auto distort_coeffs_data = yaml["distort_coeffs"].as<std::vector<double>>();
  ...
  for (int i = 1; true; i++) {
    ...
    // 计算所需的数据
    Eigen::Matrix3d R_world2gimbal = R_gimbal2world.transpose();
    cv::Mat t_world2gimbal = (cv::Mat_<double>(3, 1) << 0, 1.35, 0.13);
    cv::Mat R_world2gimbal_cv;
    cv::eigen2cv(R_world2gimbal, R_world2gimbal_cv);
    cv::Mat rvec, tvec;
    auto corners_3d_ = generate_chessboard_corners(pattern_size, square_size_mm);
